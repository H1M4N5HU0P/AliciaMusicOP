...
@Aliciabot.on_callback_query(filters.regex(pattern=r'^(playlist)$'))
async def p_cb(b, cb):
    global que    
...
@Aliciabot.on_callback_query(filters.regex(pattern=r'^(play|pause|skip|leave|puse|resume|menu|cls)$'))
@cb_admin_check
async def m_cb(b, cb):
    global que    
    qeue = que.get(cb.message.chat.id)
...
@Aliciabot.on_message(command("play") & other_filters)
async def play(_, message: Message):
    ...
    wew = usar.id
    try:
        #chatdetails = await Aliciamusic.get_chat(chid)
           ...
           for administrator in administrators:
                      if administrator == message.from_user.id:  
                          ...
                          try:
                              ...
                              await Aliciamusic.send_message(message.chat.id,"I joined this group for playing music in VC")
                              await lel.edit(
...
async def deezer(client: Aliciabot, message_: Message):
    global que
        ...
        lmoa = await client.get_chat_member(chid,wew)
           ...
           for administrator in administrators:
                      if administrator == message_.from_user.id:  
                          ...
                          try:
                              await Aliciamusic.join_chat(invitelink)
                              await Aliciamusic.send_message(message_.chat.id,"I joined this group for playing music in VC")
                              await lel.edit(
                                  "<b>helper userbot joined your chat</b>",
                              )
...
async def jiosaavn(client: Aliciabot, message_: Message):
    global que
    lel = await message_.reply("🔄 **Processing**")
    administrators = await get_administrators(message_.chat)
    chid = message_.chat.id
